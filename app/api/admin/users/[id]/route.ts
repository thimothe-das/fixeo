...
export async function PUT(
  ...
  try {
    ...
    if (!updateData.role) {
      return NextResponse.json({ error: "Rôle est requis" }, { status: 400 });
    }

    // Role-specific validation
    if (updateData.role === "professional") {
      if (
        !updateData.professionalProfile?.firstName?.trim() ||
        !updateData.professionalProfile?.lastName?.trim()
      ) {
        return NextResponse.json(
    ...
    if (updateData.role === "client") {
      if (
        !updateData.clientProfile?.firstName?.trim() ||
        !updateData.clientProfile?.lastName?.trim()
      ) {
        return NextResponse.json(
