...
export async function POST(
  ...
  try {
    ...
    if (
      !requestId ||
      !type ||
      !["success", "issue", "impossible", "validate", "dispute"].includes(type)
    ) {
    ...
    if (
      !validStatesForCompletion.includes(
...
async function sendCompletionNotifications(
  ...
  try {
    ...
    if (serviceRequestData.client?.email) {
      console.log(
        `Should notify client ${serviceRequestData.client.email} about completion: ${completionType}`
      );
    ...
    if (completionType === "dispute") {
      ...
      console.log(
        `Should notify admin ${adminEmail} about completion dispute: ${completionType}`
      );
